...
\stepcounter{probseq}
\par \textbf{\theprobseq .} (\textbf{差分序列的通项公式}) 设$p$是自然数, $\{f_n\}$是复数列, 证明:
\begin{equation}
   \Delta^p f_n = \sum_{j=0}^p (-1)^{p-j} \binom{p}{j} f_{n+j} 
\end{equation}
...
\stepcounter{solseq}
\par \textbf{\thesolseq .} 对$p$归纳, $p=0$时成立. 设命题对$p$成立, 则有
\begin{align*}
    \Delta^{p+1} f_n &= \Delta^p (f_{n+1}- f_n)= \sum_{j=0}^p (-1)^{p-j}\binom{p}{j} (f_{n+1+j} - f_{n+j})\\
    &= (-1)^{p+1}f_n + \sum_{j=1}^p (-1)^{p+1-j}\binom{p+1}{j} f_{n+j} + f_{n+p+1}\\
    &= \sum_{j=0}^{p+1} (-1)^{p+1-j}\binom{p+1}{j} f_{n+j}
\end{align*}
