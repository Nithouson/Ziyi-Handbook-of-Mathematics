...
\end{equation}
特别地, 当$p=q$时, 位于对角线$y=x$下方的HVD格路径称为\textbf{Schröder路径}, 其计数称为\textbf{大Schröder数}$R(n)$:
\begin{equation}
    R(n) = \sum_{r=0}^{n} \frac{1}{n-r+1} \frac{(2n-r)!}{r!((n-r)!)^2}
\end{equation}

\stepcounter{probenum}
\par \textbf{\theprobenum .} \cite{IC} 证明以下对象的计数结果为Catalan数$C_n$: 
\par (1) $n$个1, $n$个-1的全排列$(a_1,\dots,a_{2n})$, 满足对任意$1\le k\le 2n$, 
\begin{equation}
    \sum_{i=1}^{k} a_i \ge 0
\end{equation}
...
\stepcounter{solenum}
\par \textbf{\thesolenum .} (1) 考虑从$(0,0)$到$(n,n)$的格路径, 将1与H对应, -1与V对应, 则满足条件的排列对应位于对角线$y=x$下方的格路径.
