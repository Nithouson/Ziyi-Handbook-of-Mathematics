...
尺规作图(ruler-and-compass construction)使用无刻度的直尺和圆规作出给定图形。允许的操作包括: 过两已知点作直线；以已知点为圆心，两已知点距离为半径作圆；取位于至多两个直线或圆上的一个点。每次作直线、作圆计为一步.

\begin{pro}\textbf{作线段中垂线}(步数3)
\par 如图\ref{fig:plg_rc_midpoint}, 给定线段$AB$, 以$A$为圆心、$AB$为半径作圆A，以$B$为圆心、$AB$为半径作圆B，两圆交于C、D，作直线CD交线段$AB$于$M$, 则$AM=BM$, 且$CD\perp AB$. 
\end{pro}
\begin{proof}
由$AC=AD$, $BC=BD$, $\triangle ADC \cong \triangle BDC$, 故$\angle ACM = \angle BCM$. 又由$AC=BC$, $\triangle ACM \cong \triangle BCM$. 故$AM=BM$, $\angle AMC=\angle BMC=\frac{\pi}{2}$.
\end{proof}
...
\end{pro}
\begin{proof}
由$AB=AC$, $BD=BC=CD$, $\triangle BAD \cong \triangle CAD$. 
\end{proof}
